    ...
    <main class="schedule">
      ...
      @csrf
        @for ($i = 0; $i < sizeof($movie); $i++)
          @php
              $pickImage = substr(DB::table('movies')->where('nameMovie', $movie[$i]->nameMovie)->value('image'), 6);
          @endphp
          <section class="movie">
            <div class="movie__info">
              <div class="movie__poster">
                <img class="movie__poster-image" alt="{{$movie[$i]->nameMovie}}" src="{{url("/storage/$pickImage")}}">
              </div>
